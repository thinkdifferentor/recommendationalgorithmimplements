...
class SpreadoutSampler(Sampler):
    ...
    def _candidate_neg_ids(self, pos_ids):
        ...
        return np.random.choice(range(self.interactions.shape[1]),\
            size=self.num_neg_candidates,replace=False,p=self.item_popularities)
    ...
    def _negative_sampling(self, user_ids, pos_ids, neg_ids):
        ...
        item_spreadout_distances = np.linalg.multi_dot([pos_item_embeddings,neg_item_embeddings.T])
        ...
        for i, uid, in enumerate(user_ids):
            ...
            if weights_sum > 0:
                neg_samples[i] = np.random.choice(neg_ids,size=self.n_negatives,p=weights,replace=False)
            else:
                neg_samples[i] = np.random.choice(neg_ids,size=self.n_negatives,replace=False)

            for j, neg in enumerate(neg_samples[i]):
                current_try = 0
                while neg in self.user_items[uid] and current_try < self.n_try:
                    if weights_sum > 0:
                        neg_samples[i, j] = neg = np.random.choice(neg_ids,p=weights)
                    else:
    ...
    def _calculate_weights(self, spreadout_distance):
        ...
        log_weights = (1.0 - (float(self.embedding_dim) - 1) / 2) * \
            np.log(1.0 - np.square(spreadout_distance) + 1e-8) + np.log(BETA_VOLUMN_CONST)
