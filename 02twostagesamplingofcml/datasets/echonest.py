...
def _build_interactions(data, min_interactions):
    ...
    :return:
    """
    logger.debug('Filter users has less than {} interactions'.format(min_interactions))
    # Filter Two: Filter users has less than min_interactions & songs has less than min interacted.
    data = _filter_data(data, 'user_ID', min_interactions)
    # data = _filter_data(data, 'item_ID', min_interactions) ???
    ...
    interaction_mat = sp.dok_matrix((len(user_ids), len(item_ids)),dtype=np.float32)
...
def fetch_echonest(data_home, min_playcount=5, min_interactions=10):
    ...
    if not os.path.exists(echo_interactions_path) or \
        ...
        # read raw data
        data = pd.read_csv(echo_data_path, sep='\t',
                           names=['user_ID', 'song_ID', 'play_count'])
        # Filter One: filter out only tracks with playcount more than min_playcount.
        data = data[data.play_count >= min_playcount]
        interactions, user_ids, item_ids = _build_interactions(data, min_interactions)
        # Save the resualt.
    ...
    logger.debug('Number of interactions: {}'.format(interactions.count_nonzero()))
    logger.debug('Mapping interactions density: {}'.format(
        float(interactions.count_nonzero()) * 100 / (len(user_ids) * len(item_ids))))
    # 
    return {
