...
def sample_function(user_item_matrix, batch_size, n_negative, result_queue, check_negative=True):
    ...
    while True:
        ...
        for i in range(int(len(user_item_pairs) / batch_size)):
            ...
            # sample negative samples : the negative sampls adapt random stategy, 
            # but this have no judge about the negative samples is in the positive lists. 
            # the check step in optional at behind.
            ...
            if check_negative:
                for user_positive, negatives, i in zip(user_positive_items_pairs,
                    ...
                    for j, neg in enumerate(negatives):
                        # if ths random generate nagetive sample in the positive list, 
                        # we regenerate a nagetive sample which is not in ths positive list.
